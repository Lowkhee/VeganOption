...
public class ContentHelper
{
	...
	/*
	 * associate an item with all similar items in the oredictionary, if any exist
	 * 
	 * return null if no known alias's, otherwise all known names
	 */
	public static  NonNullList<ItemStack> associateOreDictAlias(ItemStack itemStack)
	{
		String[] oreNames = OreDictionary.getOreNames();
		NonNullList<ItemStack> itemStackOreNames = NonNullList.create();
		if(oreNames != null)
			for(String oreName : oreNames)
			{
				if(oreName.contains(itemStack.getDisplayName()))
				{
					itemStackOreNames.addAll(OreDictionary.getOres(oreName));
					for(ItemStack itemStackOreName : itemStackOreNames)
					{
						Modifiers.recipes.convertInput(itemStackOreName, itemStack.getDisplayName());
						ContentHelper.remapOre(itemStackOreName.getDisplayName(), itemStack.getDisplayName());
						ContentHelper.remapOre(itemStack.getDisplayName(), itemStackOreName.getDisplayName());
					}
				}
			}
		return itemStackOreNames;
	}
}
