...
public class BlockBasin extends Block implements IHollowBlock, IProbeInfoAccessor
{
	...
	public TileEntityBasin tileBasin = null;
	public ItemStack basinStack = null;
	...
	{
		tileBasin = (TileEntityBasin)world.getTileEntity(pos);
		boolean open = false; 
	...
	public void neighborChanged(IBlockState state, @Nonnull World world, @Nonnull BlockPos pos, Block blockChanged, BlockPos posChanged)
	{
		...
		if (tileBasin != null)
		{
			...
			if(tileBasin.getValveDirection() == EnumFacing.UP && isPowered && BlockHelper.sideBlockLocated(pos, posChanged).compareTo(state.getValue(FACING)) == 0)
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune)
    {
		...
		basinStack = new ItemStack(state.getBlock());
		if(!tileBasin.isBasinEmpty())
			basinStack.setTagCompound(tileBasin.getUpdateTag());
		drops.add(basinStack);
        return drops;
    }
    ...
    {
		//Block basin = state.getBlock();
		//EntityPlayer player = world.getClosestPlayer(pos.getX(), pos.getY(), pos.getZ(), 5, false);
		//player.inventory.addItemStackToInventory(new ItemStack(basin));
		//state = null;
		//if(basin instanceof BlockBasin)
		//	((BlockBasin)basin).tileBasin.getUpdateTag();
	...
	public void onBlockPlacedBy(@Nonnull World world, @Nonnull BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
    {
		...
		if(tag != null)
		{
			tileBasin.readFromNBT(tag);
			tileBasin.setPos(pos);
			tileBasin.setWorld(world);
		}
